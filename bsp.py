...
class LineSegment:
	...
	def compare(self, OtherLine):
		DotProduct1 = self.NormalV.dotProduct(Vector((OtherLine.p1.x - self.p1.x), (OtherLine.p1.y - self.p1.y)))
		if abs(DotProduct1) < DoubleTolerance:
			DotProduct1 = 0
			
		DotProduct2 = self.NormalV.dotProduct(Vector((OtherLine.p2.x - self.p1.x), (OtherLine.p2.y - self.p1.y)))
		if abs(DotProduct2) < DoubleTolerance:
			DotProduct2 = 0
			
		if (sign(DotProduct1) == 1 and sign(DotProduct2) == -1) or (sign(DotProduct1) == -1 and sign(DotProduct2) == 1):
			# Lines Partition
			return 'P'
	...
	def split(self, OtherLine):
		numer = (self.NormalV.x * (OtherLine.p1.x - self.p1.x)) + (self.NormalV.y * (OtherLine.p1.y - self.p1.y))	
		denom = ((-self.NormalV.x) * (OtherLine.p2.x - OtherLine.p1.x)) + ((-self.NormalV.y) * (OtherLine.p2.y - OtherLine.p1.y))
		...
		if t >= 0.0 and t <= 1.0:
			...
			IntersectPoint.x = OtherLine.p1.x + t * (OtherLine.p2.x - OtherLine.p1.x)
			IntersectPoint.y = OtherLine.p1.y + t * (OtherLine.p2.y - OtherLine.p1.y)
			return (LineSegment(OtherLine.p1, IntersectPoint, (OtherLine.Name + '1')), LineSegment(IntersectPoint, OtherLine.p2, OtherLine.Normal, (OtherLine.Name + '2')))
		else:
			return None
...
class BSP:
		...
		with open(filename, 'r') as f:
			for line in f.readlines():
				if line[0] != '#':
					...
					self.tree.data.append(LineSegment(Point(points[0], points[1]), Point(points[2], points[3]), int(data[1]), data[2][0:len(data[2])-1]))
	...
	#def heuristicEvenDivide(self, ListLineSegments):
		#change this
	#	BestIndex = 0
	#	if len(ListLineSegments) > 0:
	#		SumX = 0
	#		SumY = 0
	#		for LineSegment in ListLineSegments:
	#			MidPoint = LineSegment.getMidPoint()
	#			SumX += MidPoint.x
	#			SumY += MidPoint.y
	#			
	#		AvgPoint = Point(SumX / len(ListLineSegments), SumY / len(ListLineSegments))
	#	
	#		ClosestLineDistance = 99999999
	#		for index, LineSegment in enumerate(ListLineSegments):
	#			distance = LineSegment.getMidPoint().getDistance(AvgPoint)
		...
		for FromIndex, FromPoint in enumerate(points):
			for ToIndex, ToPoint in enumerate(points):
				...
				if (FromIndex != ToIndex) and (LoS[FromIndex][ToIndex] == 'X'):
					...
					SightSegment = LineSegment(points[FromIndex], points[ToIndex])
					...
					while len(stack) != 0 and IsIntersection == False:
						...
						if compareLoS == 'P':
							for line in TreePointer.data:
								...
								if SightSegment.split(line) != None:
									IsIntersection = True
									break
							...
							if IsIntersection:
								...
								if TreePointer.left != None:
									stack.append(TreePointer.left)
								if TreePointer.right != None:
					...
					if IsIntersection:
						print(('Distance: %0.1f' % distance) + ', # of traversals(F): ' + str(NumOfTraversals))
					else:
						print(('Distance: %0.1f' % distance) + ', # of traversals(T): ' + str(NumOfTraversals))
...
def generateRandomScene(n, width, height, MinDistance = 10, MaxDistance = 30, isUniform = True):
	Lines = []
	for i in range(n):
		Done = False
		while not Done:
			...
			if isUniform:
				...
				while not (P2x >= 0 and P2x <= width):
					c = np.random.uniform(-1, 1)
					P2x = P1x + int(round(c * Distance))
					
				while not (P2y >= 0 and P2y <= height):
					P2y = P1y + int(round(sign(np.random.uniform(-1, 1)) * (1 - (abs(c))) * Distance))
			...
			#if NewLine.getLength() >= MinDistance and NewLine.getLength() <= MaxDistance:
			IsIntersection = False
			for line in Lines:
				if NewLine.split(line) != None:
					IsIntersection = True
					break
...
def generatePoints(n, width, height, isUniform = True):
	...
	for i in range(n):
		if isUniform:
			Points.append(Point(int(round(generateRandom(1, width))), int(round(generateRandom(1, height)))))
		else:
			Points.append(Point(int(round(generateRandom(1, width, isPowerLaw = True))), int(round(generateRandom(1, height, isPowerLaw = True)))))
...
def main():
	...
	p = False
	bsptree.tree.data = generateRandomScene(1024, SCREEN_WIDTH, SCREEN_HEIGHT, isUniform = p)
	...
	for l in bsptree.tree.data:
		pygame.draw.line(windowSurface, WHITE, (l.p1.x, l.p1.y), (l.p2.x, l.p2.y), 2)
	...
	pygame.draw.circle(windowSurface, YELLOW, (int(bsptree.tree.data[0].getMidPoint().x), int(bsptree.tree.data[0].getMidPoint().y)), 4, 4)
	...
	for iFrom, From in enumerate(LoS):
		for iTo, To in enumerate(LoS):
			if iFrom != iTo and LoS[iFrom][iTo] == 'T':
				pygame.draw.line(windowSurface, GREEN, (points[iFrom].x, points[iFrom].y), (points[iTo].x, points[iTo].y))
